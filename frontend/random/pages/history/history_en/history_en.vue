	...
	export default {
		...
		methods: {
			...
			fetchHistoryData() {
				...
				uni.request({
					url: 'http://111.231.191.2:8080/api/gethistory?type=1', // 你的API端点
					method: 'GET',
					header:{
			...
			onActionButtonClick(index, item) {
				...
				const url = 'https://dictionary.cambridge.org/zhs/%E8%AF%8D%E5%85%B8/%E8%8B%B1%E8%AF%AD-%E6%B1%89%E8%AF%AD-%E7%AE%80%E4%BD%93/' +
								word;
			...
			clearHistory(){
				const token = this.getToken();
				uni.request({
					url: 'http://111.231.191.2:8080/api/clearhistory?type=1', // 你的API端点
					method: 'GET',
					header:{
						Authorization: `Bearer ${token}`
					},
